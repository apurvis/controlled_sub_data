...
describe StatuteComparisonsController do
  ...
  it 'compares across states' do
    ...
    expect(assigns(:state_one_only)).to eq(federal_statute.substance_statutes.map { |ss| ss.substance })
    expect(assigns(:state_two_only)).to eq(state_statute.substance_statutes.map { |ss| ss.substance })
  end
end
